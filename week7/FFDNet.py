...
# 全局加载FFDNet模型，避免重复加载
try:
    ffdnet_device = 'cuda' if torch.cuda.is_available() else 'cpu'
    ffdnet_model = load_ffdnet_model(device=ffdnet_device)
except Exception as e:
    print(f"Warning: Could not load FFDNet model. FFDNet will be disabled. Error: {e}")
    ffdnet_model = None
...
def ffdnet_denoise(noisy_img, clean_img, sigma=25):
    ...
    # 移动到设备
    noisy_tensor = noisy_tensor.to(ffdnet_device)
    noise_map = noise_map.to(ffdnet_device)
    ...
    if ffdnet_model is not None:
        try:
            ...
            ffdnet_denoised, ffdnet_psnr, ffdnet_ssim, ffdnet_time = ffdnet_denoise(
                noisy_img, clean_img, sigma=noise_param if noise_type == 'gaussian' else 25)
            results['FFDNet'] = {
                'img': ffdnet_denoised,
                'psnr': ffdnet_psnr,
                'ssim': ffdnet_ssim
...
def plot_results(clean_img, noisy_img, results, image_name, noise_type, noise_param, save_dir='ffdnet_results/image'):
    """可视化结果并保存"""
    plt.figure(figsize=(20, 4))

    algorithms = ['BM3D', 'DnCNN', 'FFDNet', 'ISTA', 'FISTA', 'ADMM']
...
def plot_convergence(results, image_name, noise_type, noise_param, save_dir='ffdnet_results/convergence'):
    """绘制收敛曲线并保存"""
...
def save_results_to_csv(all_results, save_path='ffdnet_results/summary.csv'):
    """保存所有结果到CSV文件"""
    data = []
...
def main():
    ...
    # 保存详细结果到Excel
    detailed_path = 'ffdnet_results/detailed_results.xlsx'
