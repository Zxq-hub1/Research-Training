...
from data_generator import batch_size
...
def sum_squared_error(y_true, y_pred):
    return tf.reduce_sum(tf.square(y_pred - y_true)) / 2
...
if __name__ == '__main__':
    ...
    checkpointer = ModelCheckpoint(os.path.join(save_dir, 'model_{epoch:03d}.keras'),
                                   verbose=1, save_weights_only=False, save_freq=10*batch_size)
    ...
    log(f'最终模型已保存到: {final_model_path}')
