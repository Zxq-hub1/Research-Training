...
import numpy as np
from train_n2n_new import SimpleNoise2Noise
...
# FFDNet模型定义
class MyFFDNet(nn.Module):
    """FFDNet模型实现"""

    def __init__(self, in_channels=1):
        super(MyFFDNet, self).__init__()
...
# ===================================================================
#  1. 加载 Noise2Noise 模型（彩色）
# ===================================================================
def load_n2n_model(device='cpu'):
    """加载 train_n2n_new.py 训练好的 SimpleNoise2Noise 彩色模型"""
    # 尝试不同的构造函数
    try:
        model = SimpleNoise2Noise(in_channels=3, out_channels=3)
    except TypeError:
        # 如果构造函数不支持 in_channels 参数，使用默认参数
        try:
            model = SimpleNoise2Noise()
        except Exception as e:
            print(f"无法创建 Noise2Noise 模型: {e}")
            return None


    cand = [
        'models/N2N_DIV2K/best.pth',      # 默认训练输出路径
        'models/N2N_DIV2K/epoch100.pth',
        'best.pth'
    ]
    loaded = False
    for ckpt in cand:
        ...
        try:
            ...
            if 'model_state_dict' in state:          # 完整 checkpoint
                state = state['model_state_dict']
            # 去 DataParallel 前缀
            from collections import OrderedDict
            clean_state = OrderedDict((k[7:] if k.startswith('module.') else k, v)
                                      for k, v in state.items())
            model.load_state_dict(clean_state, strict=True)
            print(f'✅ Noise2Noise 权重加载成功：{ckpt}')
            loaded = True
            break
        except Exception as e:
            print(f'❌ 加载 Noise2Noise 失败：{ckpt}  {e}')
            continue
    if not loaded:
        print('⚠️ 未找到 Noise2Noise 预训练权重，使用随机初始化！')
...
try:
    ...
    n2n_model = load_n2n_model(device=device)
except Exception as e:
...
# ===================================================================
#  2. Noise2Noise 彩色去噪接口
# ===================================================================

def n2n_denoise_color(noisy_img_bgr, clean_img_bgr):
    """
    输入：
        noisy_img_bgr : 噪声图  (H,W,3)  uint8  BGR
        clean_img_bgr : 干净图  (H,W,3)  uint8  BGR  （仅用于计算指标）
    返回：
        denoised_bgr  : 去噪图  (H,W,3)  uint8  BGR
        psnr, ssim, elapsed_time
    """
    if n2n_model is None:
        raise RuntimeError('Noise2Noise 模型未成功加载')

    start = time.time()
    # BGR → RGB → float32[0,1]
    noisy_rgb = cv2.cvtColor(noisy_img_bgr, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.
    # → Tensor (1,3,H,W)
    tensor = torch.from_numpy(noisy_rgb).permute(2, 0, 1).unsqueeze(0).to(device)

    with torch.no_grad():
        denoised_tensor = n2n_model(tensor)          # 输出 sigmoid 已约束在 [0,1]
    # → numpy (H,W,3)  RGB
    denoised_rgb = denoised_tensor.squeeze(0).permute(1, 2, 0).cpu().numpy()
    denoised_rgb = np.clip(denoised_rgb * 255, 0, 255).astype(np.uint8)
    # RGB → BGR
    denoised_bgr = cv2.cvtColor(denoised_rgb, cv2.COLOR_RGB2BGR)

    elapsed = time.time() - start
    psnr_val = psnr_color(clean_img_bgr, denoised_bgr, data_range=255)
    ssim_val = ssim_color(clean_img_bgr, denoised_bgr, data_range=255)
    return denoised_bgr, psnr_val, ssim_val, elapsed
...
def process_image_color(image_path, noise_type='gaussian', noise_param=25, max_iter=100, resize=True):
    ...
    # >>>>>>>  Noise2Noise 彩色 <<<<<<<<
    if n2n_model is not None:
        try:
            start = time.time()
            n2n_img, n2n_psnr, n2n_ssim, n2n_time = n2n_denoise_color(noisy_img, clean_img)
            results['Noise2Noise'] = {'img': n2n_img, 'psnr': n2n_psnr, 'ssim': n2n_ssim}
...
def plot_results_color(clean_img, noisy_img, results, image_name, noise_type, noise_param,save_dir='N2N_results2/image'):
    """可视化结果并保存（彩色版本）"""
    plt.figure(figsize=(12, 4))

    algorithms = ['BM3D', 'FFDNet', 'UNet', 'Noise2Noise', 'ADMM']
...
def main_color():
    """彩色图像去噪主函数"""
    # 创建结果目录
    os.makedirs('N2N_results2/image', exist_ok=True)
    os.makedirs('N2N_results2/metrics', exist_ok=True)
    ...
    df.to_csv('N2N_results2/metrics/color_denoising_results.csv', index=False)
    ...
    # 保存平均指标
    avg_metrics.to_csv('N2N_results2/metrics/color_avg_metrics.csv', index=False)

    print("\nProcessing completed! Results saved in 'N2N_results2/' directory")
