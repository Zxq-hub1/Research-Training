...
class CondRefineBlock(nn.Module):
    def __init__(self, in_planes, features, num_classes, normalizer, act=nn.ReLU(), start=False, end=False):
        ...
        self.output_convs = CondRCUBlock(features, 3 if end else 1, 2, num_classes, normalizer, act) # conv+norm

        if not start:
            self.msf = CondMSFBlock(in_planes, features, num_classes, normalizer) # upsampling

        self.crp = CondCRPBlock(features, 2, num_classes, normalizer, act) # maxpool
