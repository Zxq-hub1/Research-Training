...
import numpy as np
from week6.self2self.network.pconv_layer import PConv2D


def get_weight(shape, gain=np.sqrt(2)):
    fan_in = np.prod(shape[:-1])
    std = gain / np.sqrt(fan_in)
    w = tf.compat.v1.get_variable('weight', shape=shape, initializer=tf.compat.v1.initializers.random_normal(0, std))
    return w
...
def build_denoising_unet(noisy, p=0.7, is_realnoisy=False):
    ...
    is_flip_lr = tf.compat.v1.placeholder(tf.int16)
    is_flip_ud = tf.compat.v1.placeholder(tf.int16)
    ...
    slice_avg = tf.compat.v1.get_variable('slice_avg', shape=[_, h, w, c], initializer=tf.compat.v1.initializers.zeros())
    if is_realnoisy:
        response = tf.squeeze(tf.compat.v1.random_poisson(25 * response, [1]) / 25, 0)
...
def mask_loss(x, labels, masks):
    cnt_nonzero = tf.cast(tf.math.count_nonzero(masks), tf.float32)
