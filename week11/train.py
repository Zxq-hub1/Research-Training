...
parser.add_argument('--data_dir', type=str, default='/202521000855/ZXQ/project/week6/data/DIV2K_train')
parser.add_argument('--val_dirs', type=str, default='/202521000855/ZXQ/project/week6/data/DIV2K_valid')
parser.add_argument('--save_model_path', type=str, default='./neighbor_results')
...
class DataLoader_Imagenet_val(Dataset):
    def __init__(self, data_dir, patch=256):
        ...
        self.train_fns = []
        for e in ext:
            self.train_fns.extend(glob.glob(os.path.join(self.data_dir, e)))
        self.train_fns.sort()
    ...
    def __getitem__(self, index):
        ...
        im = Image.open(fn)
        if im.mode != 'RGB':
            im = im.convert('RGB')  # 灰度->RGB
        im = np.array(im, dtype=np.float32)  # 现在一定是 (H, W, 3)
        # random crop
        H = im.shape[0]
        W = im.shape[1]
        if H - self.patch > 0:
...
# 3. 新建一个专门加载 DIV2K_valid 的函数（与原 validation_kodak 几乎一样）
def validation_div2k(dataset_dir):
    fns = glob.glob(os.path.join(dataset_dir, "*"))
    fns.sort()
    images = []
    for fn in fns:
...
# Validation Set
DIV2K_valid_dir = opt.val_dirs
valid_dict = {"DIV2K_valid": validation_div2k(DIV2K_valid_dir)}
...
#加载预训练
dir = torch.load("./epoch_model_085.pth")
network.load_state_dict(dir)
...
for epoch in range(1, opt.n_epoch + 1):
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        np.random.seed(101)
        valid_repeat_times = {"DIV2K_valid": 1}
...
print("Training   set:", opt.data_dir)
print("Validation set:", opt.val_dirs)