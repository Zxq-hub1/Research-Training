...
def create_simple_dncnn():
    """创建一个简单的DnCNN结构（无预训练权重）"""
    ...
    model.save('models/dncnn.keras')
    return model


# 如果模型不存在，则创建
if not os.path.exists('models/dncnn.keras'):
    create_simple_dncnn()
...
def load_dncnn_model():
    """加载预训练的DnCNN模型"""
    # 模型文件位于当前目录下的models文件夹中
    model_path = os.path.join('models', 'dncnn.keras')
...
def plot_results(clean_img, noisy_img, results, image_name, noise_type, noise_param, save_dir='results/image'):
    """可视化结果并保存"""
    plt.figure(figsize=(18, 4))

    algorithms = ['BM3D', 'DnCNN', 'ISTA', 'FISTA', 'FISTA_TV', 'ADMM']
...
def plot_convergence(results, image_name, noise_type, noise_param, save_dir='results/convergence'):
    """绘制收敛曲线并保存"""
...
def main():
    ...
    dataset_path = '/project/week5/Set14'
